...
dependencies {
    ...
    testImplementation("org.mockito:mockito-inline:5.2.0") // バージョンは適宜
...
tasks.withType<Test> {
    useJUnitPlatform()
    jvmArgs?.addAll(
        listOf("-javaagent:${configurations.testRuntimeClasspath.get().files.find { it.name.contains("mockito-inline") }}"),
    )
}
